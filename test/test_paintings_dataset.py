...
from dataloaders.PaintingDatasets import (
    PaintingsDataset,
    compute_weights,
    CreatePaintingsDataLoaders,
)
...
def test_dataset_length(sample_csv_with_images):
    ...
    assert (
        len(dataset) == 3
    ), "Dataset length should match the number of rows in the CSV."
...
def test_transform(sample_csv_with_images):
    ...
    dataset = PaintingsDataset(
        csv_file=csv_file, data_folder=image_dir, transform=transform
    )
    image, labels = dataset[0]
...
def test_dataloaders(sample_csv_with_images):
    ...
    csv_file, image_dir = sample_csv_with_images
    batch_size = 2
    train_loader, val_loader, weights, unique_classes = CreatePaintingsDataLoaders(
        csv_file=csv_file, data_folder=image_dir, batch_size=batch_size
    )
    ...
    ), "Validation loader should be a DataLoader."

    # Check weights
    assert (
        len(weights) == 3
    ), "Weights list should have 3 elements for 'style', 'date', and 'type'."

    # Check unique classes
    assert (
        len(unique_classes) == 3
    ), "Unique classes should have 3 elements for 'style', 'date', and 'type'."
    assert (
        "Impressionism" in unique_classes[0]
    ), "Unique classes for 'style' should include 'Impressionism'."
    assert (
        "1550 - 1650" in unique_classes[1]
    ), "Unique classes for 'date' should include '1550 - 1650'."
    assert (
        "landscape" in unique_classes[2]
    ), "Unique classes for 'type' should include 'landscape'."
...
def test_compute_weights(sample_csv_with_images):
    ...
    assert (
        len(style_weights) == 2
    ), "Style weights should have the same number of unique classes."
