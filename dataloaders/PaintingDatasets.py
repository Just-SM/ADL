...
def compute_weights(dataset, column):
    class_weights = compute_class_weight(
        "balanced", classes=np.unique(dataset.data[column]), y=dataset.data[column]
    )
...
class PaintingsDataset(Dataset):
    def __init__(self, csv_file, data_folder, transform=None):
        self.data = pd.read_csv(
            csv_file
        )  # CSV with columns: ['image_path', 'style', 'date', 'type']
...
def CreatePaintingsDataLoaders(
    csv_file,
    data_folder,
    batch_size=32,
    ...
    dataset = PaintingsDataset(
        csv_file=csv_file, data_folder=data_folder, transform=transformer_pipe
    )

    val_dataset = PaintingsDataset(
        csv_file=csv_file, data_folder=data_folder, transform=transformer_pipe
    )
    ...
    return (
        train_loader,
        val_loader,
        [style_weights, date_weights, type_weights],
        [
            dataset.data["style"].unique(),
            dataset.data["date_bin"].unique(),
            dataset.data["type"].unique(),
        ],
    )
