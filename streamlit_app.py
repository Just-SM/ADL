...
def prep_images_nn():
    ...
    return NearestNeighbors(metric='cosine',algorithm='brute').fit(st.session_state['images_emb'])
...
def check_for_exact_match(sim):
    ...
    return sim[0] < 0.18 and abs(sim[0] - sim[1]) > sum([ abs(x-y) for x,y in zip(sim[1:],sim[2:])]) / len(sim-2)
...
def load_model():
    ...
    model.load_state_dict(torch.load(os.path.join('data', 'model.pth'),weights_only=True))
...
image = st.file_uploader("Upload image", ['png','jpg'],accept_multiple_files=False,)


if image:
    
    image_tensor = decode_image(torch.tensor(bytearray(image.getvalue()),dtype=torch.uint8),mode=ImageReadMode.RGB)
    ...
    if check_for_exact_match(dist[0]):
        with col3:
            ...
            st.markdown(f" ## {st.session_state['images_data'].iloc[ind[0][0]]['title']} ")
            st.markdown(f" ### {st.session_state['images_data'].iloc[ind[0][0]]['author']} ")
    ...
    date = sorted(Counter([ bin_dates(x) for x in date]).items(),key=lambda x: x[1],reverse=True)
    ...
    for i,val in zip(ind[0],dist[0]):
        ...
        with c2:
            ...
            st.markdown(f" #### Date: *{int(st.session_state['images_data'].iloc[i]['date'])}* ")
            st.markdown(f" #### Style: *{st.session_state['images_data'].iloc[i]['style']}* ")
