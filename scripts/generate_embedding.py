...
model.load_state_dict(torch.load(os.path.join(DATA_FOLDER, 'model.pth',),weights_only=True))
model.to(device)
...
for id in tqdm(df['index']):
    ...
    with torch.no_grad():
        ...
        img = decode_image(os.path.join(os.path.join(DATA_FOLDER, IMAGE_RAW_DATA,),f"{id}.jpg"))
...
torch.save(val_dict, os.path.join(DATA_FOLDER, 'embeddings.pth',) )